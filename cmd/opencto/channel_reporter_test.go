...
func TestChannelReporterRoutesByChannelType(t *testing.T) {
	...
	telegramReporter := &captureChannelReporter{}
	reporter := newChannelReporter(localReporter, map[domain.ChannelType]activities.Reporter{
		...
		domain.ChannelTypeTelegram: telegramReporter,
	})
	...
	if _, err := reporter.Report(context.Background(), domain.Event{ChannelType: domain.ChannelTypeTelegram}, domain.ReportMessage{Text: "telegram"}); err != nil {
		t.Fatalf("report telegram: %v", err)
	}
	...
	if telegramReporter.reports != 1 {
		t.Fatalf("expected one telegram report, got %d", telegramReporter.reports)
	}
}

func TestChannelReporterTypingOnlyUsesConfiguredChannelReporters(t *testing.T) {
	t.Parallel()
	...
	if err := reporter.NotifyTyping(context.Background(), domain.Event{ChannelType: domain.ChannelTypeTelegram}); err != nil {
		t.Fatalf("notify telegram typing: %v", err)
	}
	...
	if telegramReporter.typing != 1 {
		t.Fatalf("expected one telegram typing notification, got %d", telegramReporter.typing)
	}
}
