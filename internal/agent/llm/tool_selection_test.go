...
func TestRenderToolSelectionPromptUsesRenderedSystemPrompt(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Known Facts: deployment_target: staging",
		"Inbound request: deploy the app",
		"Execution cycle: 2",
		"Plan summary: Deploy the app safely.",
		"Plan assumptions: Use the existing deployment workflow",
		"Plan risks: Credentials may be missing",
		"Execution order: [Verify deployment target] -> [Deploy application]",
		"Test strategy: Check the deployed app after rollout.",
		"Plan steps: Verify deployment target: Confirm the platform before deployment.",
		"Work items: Deploy application [id=wi-1,risk=2,status=ready]: Run the deployment command (acceptance=Deployment status is Ready; rollback=Rollback to the previous version.; skills=vercel; depends_on=Verify deployment target; complexity=M)",
		"Work item: wi-1",
		"Tool: shell",
		"Observation: staging target not found",
	} {
