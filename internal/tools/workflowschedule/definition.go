...
The tool creates or updates a workflow bundle under $OPENCTO_WORKSPACE/.opencto/workflows/{workflow_id}, commits it to the bundle's Git repo, and creates a Temporal schedule that runs the approved commit_hash.
...
Workflow logic must live in files under src/. The manifest only defines durable step boundaries. Use command and args arrays for each step.
