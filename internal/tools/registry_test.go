...
func TestDefinitionsIncludeDedicatedTools(t *testing.T) {
	...
	if len(definitions) != 9 {
		t.Fatalf("expected nine tool definitions, got %d", len(definitions))
	}
...
func TestLLMDefinitionsUseCommandNameAndDescription(t *testing.T) {
	...
	if len(definitions) != 9 || definitions[0].Function == nil {
		t.Fatalf("expected nine function definitions, got %#v", definitions)
	}
