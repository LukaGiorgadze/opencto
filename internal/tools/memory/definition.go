...
const (
	...
	ProposeUpdateToolDescription = "Proposes updating an existing durable OpenCTO memory by memory_id. Use after searching when a remembered fact, preference, scope, tags, pinning, or confidence should change without deleting and recreating it. The backend validates, saves, and re-embeds accepted proposals. Keep the updated memory atomic: do not merge unrelated facts into an existing memory, and use separate memories for different scopes or primary kinds."
