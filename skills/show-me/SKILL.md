...
This skill's references and scripts live under ``.
...
- `python "scripts/winctl.py" screenshot [--front] <AppName> [output.png]` — capture the first matching app window. Use `--front` to restore/activate before capture. If no output path is given, the helper saves under `$OPENCTO_WORKSPACE/screenshots/`.
